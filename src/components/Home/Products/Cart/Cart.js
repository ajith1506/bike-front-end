...
export default function Cart(props) {
  ...
  const total = cart.reduce(
    (initialPrice, allFoodItem) => initialPrice + allFoodItem.price,
    0
  );
  console.log(total);
    ...
    <section className="border p-2 rounded shadow">
      ...
      <div className="d-flex flex-row justify-content-between align-items-start mt-3 mb-3">
        ...
        <div className="d-flex flex-column justify-content-even w-50">
          <span className="ms-auto" id={style.dollar}>
            ${total}
          </span>
          <small className="ms-auto" id={style.dollar}>
            $0
          </small>
        </div>
      </div>
        ...
        {user ? (
          <Link to="/pdDetails">
            <button onClick={handleSubmit} className={style.btnAdd}>
              checkout{" "}
              <FontAwesomeIcon icon={faArrowRight} className={style.CheckBtn} />
            </button>
          </Link>
        ) : (
          <button onClick={toastify} className={style.btnAdd}>
            checkout{" "}
            <FontAwesomeIcon icon={faArrowRight} className={style.CheckBtn} />
          </button>
        )}
