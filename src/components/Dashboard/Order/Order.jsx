    ...
    const fetchData = async () => {
      try {
        const res = await axios.get(
          `https://capstone-backend-8tak.onrender.com/order/orderedByEmail?email=` +
            user?.email
        );
        // console.log(res)
        setDeta(res.data);
    ...
    <>
      <div className="order__container shadow">
        <div className="titleText">
          <h2 className="title">
            My<span className="titleHalf"> Orders</span>
          </h2>
        </div>
        ...
        <div className="ordered__user">
          ...
          <div className="ol__right">
            ...
            <p className="olrght__data">
              {data.map((item, index) => (
                <span key={index}>{item.cardName} ,</span>
              ))}
            </p>
            <p className="olrght__data">
              {data.map((item, index) => (
                <span key={index}>{item.cardNumber} ,</span>
              ))}
            </p>
            <p className="olrght__data">
              {data.map((item, index) => (
                <span key={index}>{item.cvc} ,</span>
              ))}
            </p>
          </div>
        </div>
          ...
          {data.map((Sitem, index) => (
            <tbody key={index}>
              <tr>
                ...
                <td data-title="Product Name" className="text-muted fw-bold">
                  {Sitem.pdName}
                </td>
                ...
                <td data-title="Action">
                  {Sitem ? (
                    <button
                      className="orderBtn__delete"
                      onClick={() => handleDeleteOrder(Sitem._id)}
                    >
                      <FontAwesomeIcon icon={faTrash} />
                    </button>
                  ) : (
                    <button className="orderBtn__delete">Not ordered</button>
                  )}
                </td>
              </tr>
            </tbody>
