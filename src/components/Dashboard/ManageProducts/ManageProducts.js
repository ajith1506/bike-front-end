...
const ManageProducts = () => {
  ...
  console.log(service);
  useEffect(() => {
    const url = `https://capstone-backend-8tak.onrender.com/product/get`;
    fetch(url)
      .then((res) => res.json())
  ...
  const handleDeleteService = async (_id) => {
    try {
      const res = await axios.delete(
        `https://capstone-backend-8tak.onrender.com/product/delete/${_id}`,
        config
      );
      res &&
        Swal.fire({
    ...
    <section className="mt-5 pt-5 shadow">
      <div className={style.titleText}>
        <h2 className={style.title}>
          Manage<span className={style.titleHalf}> Products</span>
        </h2>
      </div>
                ...
                <div className="card-body">
                  ...
                  <div className="editAndDlt d-flex justify-content-between align-content-center">
                    <button className={style.btnEdit}>
                      Edit{" "}
                      <FontAwesomeIcon className={style.arrow} icon={faEdit} />
                    </button>
                    <button
                    ...
                    >
                      Delete
                      <FontAwesomeIcon className={style.arrow} icon={faTrash} />
                    </button>
                  </div>
                </div>
                <svg
