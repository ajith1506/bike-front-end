...
export default function AddProducts() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = axios.post(
        "https://capstone-backend-8tak.onrender.com/product/add",
        serviceObj,
        config
      );
      console.log(res);
  ...
  return (
    <section
      className="container mt-3 pt-3 mb-5 shadow rounded"
      id={style.AddProducts}
    >
      <div className={style.titleText}>
        <h2 className={style.title}>
          Add<span className={style.titleHalf}> Products</span>
        </h2>
      </div>
