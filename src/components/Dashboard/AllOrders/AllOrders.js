    ...
    const fetchData = async () => {
      try {
        const res = await axios.get(
          "https://capstone-backend-8tak.onrender.com/order/get",
          config
        );
        // console.log(res)
        setDeta(res.data);
  ...
  const handleDeleteOrder = async (_id) => {
    try {
      const res = await axios.delete(
        `https://capstone-backend-8tak.onrender.com/order/delete/${_id}`
      );
      res &&
        Swal.fire({
          icon: "success",
  ...
  const handleStatusUpdate = async (e) => {
    ...
    try {
      const res = await axios.put(
        `https://capstone-backend-8tak.onrender.com/order/update/${itmID}`,
        upStatusObj,
        config
      );
      res &&
      ...
      <div className="shadow border rounded" id={style.order__container}>
        <div className={style.titleText}>
          <h2 className={style.title}>
            Manage<span className={style.titleHalf}> All Orders</span>
            <hr />
          </h2>
        </div>
          ...
          <div className={style.ol__right}>
            ...
            <p className={style.olrght__data}>
              <img
                src={user.img}
                alt="user-img"
                className={style.olright__img}
              />
            </p>
            <p className={style.olrght__data}>
              {decodedTkn.isAdmin ? "Admin" : "null"}
            </p>
          </div>
        </div>
        <hr />
        ...
        <table className="table border table-hover">
          ...
          {data.map((item, index) => (
            <tbody key={index}>
              <tr>
                <th scope="row" className="text-muted fw-bold">
                  1
                </th>
                <td data-title="User Name" className="text-muted fw-bold">
                  {item?.username}
                </td>
                <td data-title="Service Name" className="text-muted fw-bold">
                  {item?.pdName}
                </td>
        ...
        <div className="d-flex justify-content-between align-items-center mb-2">
          ...
          {itmupdate ? (
            <button
              ...
              className={style.orderBtn__erase}
            >
              <FontAwesomeIcon icon={faEraser} />
            </button>
          ) : (
        ...
        {itmupdate ? <button className="btn btn-primary">Update</button> : " "}
      </div>
    </>
  );
};
