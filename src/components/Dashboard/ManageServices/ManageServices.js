...
const ManageServices = () => {
  ...
  console.log(service);
  useEffect(() => {
    const url = `https://capstone-backend-8tak.onrender.com/service/get`;
    fetch(url)
      .then((res) => res.json())
  ...
  const handleDeleteService = async (_id) => {
    try {
      const res = await axios.delete(
        `https://capstone-backend-8tak.onrender.com/service/delete/${_id}`,
        config
      );
      res &&
        Swal.fire({
    ...
    <section className="mt-5 pt-5 shadow">
      <div className={style.titleText}>
        <h2 className={style.title}>
          Manage<span className={style.titleHalf}> Services</span>
        </h2>
      </div>
